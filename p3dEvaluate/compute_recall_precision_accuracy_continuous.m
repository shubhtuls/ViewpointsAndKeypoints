...
function [recall, precision, accuracy, ap, aa] = compute_recall_precision_accuracy_continuous(cls, viewThresh,methodId)
...
filename = sprintf('data/%s_cont_%s.mat', methodId, cls);
    ...
    for j = 1:num(i)
        ...
        if isempty(bbox) == 0
            ...
            if maxo >= 0.5 && det(index) == 0
                ...
                det(index) = 1;
                % check viewpoint
                diff = abs(view_pr - view_gt(index));
